...
- Public/community edition planning uses a local AI-authored creative draft plus a hosted normalizer. Configure `SONOL_PLANNER_DRIVER=remote_http` together with the hosted normalizer URL, ticket URL, and bearer token before using `present-proposal.mjs` or `recommend-plan.mjs`.
